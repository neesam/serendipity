...
def getCurrentlyListeningIds():
    ...
    currently_listening_playlist_res = json.loads(
    ...
    currently_listening_table_res = json.loads(
        supabase.schema("music_tables")
        .table("album_currentlyListening")
        .select()
        .execute()
        .model_dump_json()
    )["data"]
    ...
    table_albums = set(
        i["title"]
        for i in currently_listening_table_res
        if i["original_table"] not in ("album_vinyls", "artist_underground")
    )

    new = table_albums - playlist_albums

    if new:
        currently_listening_ids = [
            {"album": i, "tracks": [], "album_id": ""} for i in new
        ]
        ...
        for i in currently_listening_ids:
            ...
            if " - " in i["album"] and len(i["album"].split(" - ")) == 2:
                ...
                if "&" in i["album"].split(" - ")[0]:
                    for index, album in enumerate(candidates):
                        if (
                            i["album"].split(" - ")[0].split(" & ")[0].lower()
                            == album.split(" - ")[1]
                            or i["album"].split(" - ")[0].split(" & ")[1].lower()
                            == album.split(" - ")[1]
                        ):
                            if i["album"].split(" - ")[1].lower() == "".join(
                                album.split(" - ")[0].split(" - ")
                            ) or " ".join(
                                i["album"].split(" - ")[1].lower().split("-")
                            ) == "".join(
                                album.split(" - ")[0].split(" - ")
                            ):
                                album_id = search_result["albums"]["items"][index]["id"]
                ...
                for index, album in enumerate(candidates):
                    if (
                        i["album"].split(" - ")[0].lower()
                        == album.split(" - ")[0].lower()
                        or i["album"].split(" - ")[0].lower()
                        == album.split(" - ")[1].lower()
                        or i["album"].split(" - ")[1].lower()
                        == album.split(" - ")[0].lower()
                        or i["album"].split(" - ")[1].lower()
                        == album.split(" - ")[1].lower()
                    ):
                ...
                for index, album in enumerate(candidates):
                    if (
                        i["album"] == album.split(" - ")[1]
                        or i["album"].lower() == album.split(" - ")[1]
                        or i["album"].title() == album.split(" - ")[1]
                    ):
                        album_id = search_result["albums"]["items"][index]["id"]
                        break
                else:
                    to_remove.append(
                        {"album": i["album"], "tracks": [], "album_id": ""}
                    )
                    continue
            ...
            try:
                ...
                logging.exception(
                    f"Failed to retrieve album tracks for {i['album']}: {e}"
                )
...
def insertIntoCurrentlyListeningPlaylist():
    ...
    try:
        for entry in currently_listening_ids:
            for track in entry["tracks"]:
                supabase.schema("music_tables").table(
                    "album_spotifyPlaylistIds"
                ).insert(
                    {
                        "album": entry["album"],
        ...
        logging.exception(
            f"Failed to upload tracks to album_spotifyPlaylistIds for {entry['album']}: {e}"
        )

    try:
        for entry in currently_listening_ids:
            sp.playlist_add_items(
                playlist_id="2BHeysCh0gYOjVIH7pU6uy", items=entry["tracks"]
            )
    except Exception as e:
        logging.exception(
            f"Failed to add tracks to currently listening playlist for {entry['album']}: {e}"
        )
    ...
    try:
        ...
        logging.exception(
            f"Failed to upload tracks to currently listening albums playlist: {e}"
        )
