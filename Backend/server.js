...
app.get("/api/artist_underground", async (req, res) => {
    const sqlQuery = `select * from ${BQ_PROJECT}.${MUSIC_TABLES_DATASET}.artist_underground order by rand() limit 1`;
...
app.get("/api/specificMusicEntry/:title/:table", async (req, res) => {
    ...
    const query = `select * from ${BQ_PROJECT}.${MUSIC_TABLES_DATASET}.${table} where title = @title`;
